    ...
    user.findOne({email: decodedEmail}, function (err, docs) {
        event.create({
            ...
            if(err){
                ...
                productIdHashed = `${eventDocs._id}`;
                eventOwnerEmail = `${eventDocs.ownerEmail}`;
                res.send(eventDocs);
            }
...
module.exports.editeventUpdate = function (req, res){
    dotenv.config();
    var userToken = req.headers['x-access-token'];
    if (!userToken) return res.send("no token provided");
    ...
    editevent.findOne({$or: [{originalId: req.body.params.originalId}, {_id: req.body.params.originalId}]}, function (err, toBeUpdatedProduct) {
        if (err) {
            console.error(err)
        } else {
...
//lf To be updated
                ...
                if (editedFields.reviewStatus === "Pending") {
                    if (toBeUpdatedProduct) {
                        ...
                        editevent.updateOne({$or: [{originalId: req.body.params.originalId}, {_id: req.body.params.originalId}]}, {
                            location: req.body.params.location,
                            phone_number: req.body.params.phoneNumber,
                            description : req.body.params.description,
                            listingStatus: req.body.params.listingStatus,
                            ...
                            event.updateOne({_id: req.body.params.originalId}, {
                                reviewStatus: req.body.params.reviewStatus,
                                listingStatus: req.body.params.listingStatus
                        ...
                        editevent.create({
                            ...
                            editevent.updateOne({_id: productDocs._id}, {encodedAvatarUrl: encodedId}, function (err, found) {
                                if (err) {
                    ...
                    if (toBeUpdatedProduct) {
                        event.updateOne({_id: toBeUpdatedProduct.originalId}, {
                            editedVersion: false,
                            reviewStatus: req.body.params.reviewStatus,
                            listingStatus: req.body.params.listingStatus
                            ...
                            if (versionChangedDocument) {
                                editevent.deleteOne({_id: req.body.params.originalId}, function (err, deleletSucess) {
                                    if (err) {
                                        console.error(err)
                                    } else {
                    ...
                    if (toBeUpdatedProduct) {
                        ...
                        event.findOneAndUpdate({_id: req.body.params.originalId}, {
                            location: req.body.params.location,
                            phone_number: req.body.params.phoneNumber,
                            description : req.body.params.description,
                            listingStatus: req.body.params.listingStatus,
...
module.exports.uploadProductImage = function (req, res) {
    ...
    var storage = multer.diskStorage({
        ...
        destination: './public/images/products/' + eventOwnerEmail + '/' + productIdHashed,
